...
(async () => {
  const idl = require("/Users/shooter/solana/staking/solana-programs/escrow/target/idl/escrow.json");

  // const connection = new Connection("http://127.0.0.1:8899", "processed");

  const connection = new Connection("http://127.0.0.1:8899");

  // 7d4gKJDWENL6um5hM3MH8UFoYnaZqMnmQeWk2V9yRARA
  ...
  const Program_ID = new PublicKey(
    "6YeT8tkXCowfZqxpEYmPzHoJWGT7PEcifHAnoGiwM5Fk"
  );
  const program = new anchor.Program(idl, Program_ID, provider);


  // spl-token create-account GnLAhbtbGLd2ry2PceBDRX8mH3j8cFHXPfxsoV3vZgzo
// Creating account HV7K23NqhsyH3HPx9yK8otpMrvUtHe4PZTwe93FtjxrL
  const mintAddress = new PublicKey(
    // "9eDqPBqxgyQ1vd1muQF8vihswnr99HfwJsEczEsLnBsy"
    "GnLAhbtbGLd2ry2PceBDRX8mH3j8cFHXPfxsoV3vZgzo"
  );
